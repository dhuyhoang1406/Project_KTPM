...
import styles from "./AnalystSale.module.scss"
...
import { getIncomeAnalyst } from "../../../services/AnalystService";
...
const AnalystSale = ()=>{
  ...
  const convertDataIncome = (rawData) => {
    let analyst = {
        totalPrice:0,
        totalAmount:0,
        data:{
    ...
    for(let i = 0 ; i < rawData.length ;i++){
        ...
        analyst.data.datasets[0].data.push(raw.tongDoanhThu.tongDoanhThu)
        analyst.totalPrice += raw.tongDoanhThu.tongDoanhThu
        analyst.totalAmount += raw.tongDoanhThu.soLuongBanDuoc
    }
    return analyst
}
  ...
  useEffect(()=>{
      if(isStartDateBeforeEndDate(startDate,endDate)){
        getIncomeAnalyst(formatDate(startDate),formatDate(endDate)).then((res)=>{
          setContent(convertDataIncome(res.data))
        })
      }else{
    ...
    return (
        <div className={styles.wrapper}>
            ...
            <div className={styles.boxFeature}>
                ...
                <p style={{fontSize:"1.3rem",marginLeft:"auto",color:"rgb(100,100,100)",fontWeight:"700"}}>Mặc định được thống kê từ ngày 3/2/2024</p>
            </div>
            <div className={styles.boxTable}>
                <div style={{display:'flex',gap:'1.5rem'}}>
                    <div style={{display:'flex',alignItems:'center',gap:"1rem",backgroundColor:"#e91e63",width:"62rem",padding:"1rem"}}>
                      ...
                      <div>
                        <p style={{color:'white',fontWeight:'700'}}>Tổng Thu Nhập</p>
