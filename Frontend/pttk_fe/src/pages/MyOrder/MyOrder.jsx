...
const MyOrder = () => {
  ...
  const handleCancelOrder = async (id) => {
    ...
    console.log(account)
    try {
      const res = await OrderService.cancelOrder(id,{trangThai:'Huy'},account)
  ...
  return (
    <WrapperContainer>
      ...
      <div style={{display:'flex', columnGap:'1rem', alignItems:'center', userSelect:'none'}}>
      {detail && <span
        ...
        {detail ? (
                ...
                return (
                  <div key={index} style={{display:'flex', flexDirection:'column'}}>
                    <StepComponent 
                      items={itemsDelivery}
                      current={
                        product.trangThaiMoiNhat === 'ChoDuyet' ? 1 : product.trangThaiMoiNhat === 'DaDuyet' ? 2 : product.trangThaiMoiNhat === 'GiaoThanhCong' ? 3 : 4
                      }
                    />
                    {product.danhSachCTDH.map((item, itemIndex) => (
                      <OrderDetails key={index + "-" + itemIndex} product={item} />
                    ))}
                  </div>
                );
              }
          ...
          <WrapperListOrder>
                  {data && data.length > 0 ? data.map((item, index) => (
                          ...
                          {item.danhSachCTDH.map((product, id) => renderProduct(product))}
                          <WrapperFooterItem>
                            <div>
                              <span style={{ color: "rgb(255, 66, 78)" }}>
                                Tổng tiền:{" "}
