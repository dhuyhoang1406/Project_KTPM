...
export const loginUser = async (data) => {
  const res = await axiosJWT.get(
    `${process.env.REACT_APP_API_URL}/auth/login/1`,
    {
      headers: {
...
export const signupUser = async (data) => {
  try {
    const res = await axios.post(
      `${process.env.REACT_APP_API_URL}/TaiKhoan`,
      data
    );
    return res;
  } catch (error) {
...
export const getDetailsUser = async (id) => {
  const res = await axios.get(
    `${process.env.REACT_APP_API_URL}/TaiKhoan/${id}`,
    {
      headers: {
        Authorization: JWT,
      },
...
export const deleteUser = async (id) => {
  try {
    await axiosJWT.put(
      `http://localhost:8080/api/v1/TaiKhoan/globalBan/${id}`,
      {
        headers: {
          Authorization: JWT,
        },
...
export const getAllUser = async (page, size, role, search) => {
  const res = await axios.get(
    `http://localhost:8080/api/v1/TaiKhoan?sort=vaiTro&pageNumber=${page}${
      size && `&size=${size}`
    }${role ? `&vaiTro=${role}` : ""}${search ? `&search=${search}` : ""}`
  );
  return res;
};
...
export const changePassword = async (id, data) => {
  const res = await axiosJWT.put(
    `${process.env.REACT_APP_API_URL}/TaiKhoan/newPass/${id}`,
    data,
    {
      headers: {
        Authorization: JWT,
